...
//resume
import ChanHokWanTakkyResume from '../img/TakkyChan_Resume.pdf';
...
export default function Home(props) {
  ...
  const [parallex, set] = useSpring(() => ({
    xy: [0, 0],
    config: { mass: 15, tension: 1500, friction: 100 },
  }));
  const [parallex2, set2] = useSpring(() => ({
    xy: [0, 0],
    config: { mass: 15, tension: 1500, friction: 100 },
  }));
  const calc1 = (x, y) => [
  ...
  useEffect(() => {
    TweenLite.from(greetings.current, 0.8, {
      opacity: 0,
      y: -5,
      x: -100,
      delay: 0.4,
    ...
    TweenLite.from(introMsg.current, 0.8, {
      opacity: 0,
      y: -5,
      x: 100,
      delay: 0.8,
    ...
    TweenLite.from(dynamic.current, 0.8, {
      opacity: 0,
      y: 100,
      delay: 1.2,
      ease: Power3.easeInOut,
  ...
  return (
    <>
      <section id="home" className="home-section">
        <Header theme={theme} />
        <div
          className="cloudLeft-cta"
          onMouseMove={({ clientX: x, clientY: y }) => set({ xy: calc1(x, y) })}
        >
          {' '}
          <animated.img
            src={cloud}
            alt=""
            style={{ transform: parallex.xy.interpolate(trans1) }}
            className="cloud-img left"
          />{' '}
        </div>
        <div
          className="cloudLeft-cta"
        ...
        >
          ...
          <animated.img
            src={cloud}
            alt=""
            style={{ transform: parallex2.xy.interpolate(trans2) }}
            className="cloud-img right"
          />{' '}
        </div>
          ...
          <div className="intro-msg">
            <div className="intro-card" ref={introMsg}>
              ...
              <p className="card-body">
                Hi there, welcome to my profile website. I am Takky, a Full
                Stack Web Developer who are passionated, creative and a fast
                learner. I always keep update and equip myself with the latest
                technology and new features. Techs I am actively using is
                React.js and Node.js, If you are interested in my work, please
                continue to explore OR
              </p>
              <div className="card-footer">
                <a
                  href={ChanHokWanTakkyResume}
                  download="(CV)ChanHokWanTakky"
                  target="_blank"
                  className="resume"
                >
